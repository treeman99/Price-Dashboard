...
/**
 * 확정 번호를 붙여넣기 좋은 평문으로. **번호 줄만** — 한 줄에 한 세트, `a, b, c, d, e, f`.
 *
 * ⚠️ 머리말도 세트 번호도 붙이지 않는다. 2026-08-02 사용자 요청으로 둘 다 뺐다.
 * 처음엔 `1236회차 예상 번호` 머리말 + `1. ` 접두사를 붙였는데(화면과 같게), 붙여넣는 쪽에서
 * 둘 다 잡음이었다. 화면은 세트를 세로로 훑기 위해 번호가 필요하고 어느 회차인지도 보여야
 * 하지만, 복사본은 **다른 곳에 숫자를 옮겨 담는 것**이 목적이라 요구가 정반대다.
 * 세트 순서는 줄 순서로 보존되고, 회차는 화면에 떠 있으므로 정보 손실도 없다.
 *
 * ⚠️ 이 함수를 "사람이 읽는 요약"으로 재사용하지 마라. 그 용도가 생기면 별도 함수를 만들어라 —
 * 여기에 맥락을 도로 넣으면 붙여넣기가 다시 망가진다.
 *
 * UI 가 아니라 여기 두는 이유는 순수 함수라 테스트 대상이기 때문이다(web/ 은 러너 밖).
 */
export function upcomingToText(u: LottoUpcoming): string {
  return u.sets.map((set) => set.join(", ")).join("\n");
}
