...
### 에누리 폴백 — 실측 완료(2026-06)

- robots 친화적(`Allow:/`, Crawl-delay 1s, ClaudeBot 허용, ajax 디스얼로우 없음, `/detail.jsp` 허용).
- ⚠️ **에누리는 쿠팡 개별가/로켓을 깔끔히 노출하지 않는다.** 상세 SSR에 "쿠팡" 0회, 쿠팡 토큰은 기획전(`coupangexh.jsp`)·광고(`ad_coupang`)뿐 — 다나와 `cmpnyc=TP40F` 같은 판매처 가격 행이 없음 → **에누리 coupang=null 고정.**
- ✅ **전체 최저가는 매우 안정적으로 노출**: JSON-LD `"lowPrice"`(1차) / og:description `최저가 N원`(2차), 상품명 og:title.
- **구현 결정**: 에누리 = 다나와 차단 시 **전체최저가 시계열 유지 폴백**(coupang은 llm-websearch 단계가 담당). `feat/enuri-source` 브랜치, `parseEnuriDetail` + fetch 구현, 라이브 HTML 검증 + 단위테스트 8종.
