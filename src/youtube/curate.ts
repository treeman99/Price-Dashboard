...
/**
 * 채널 발굴 1회 예산(ms). **탭 예산(config.youtubeTimeoutMs)과 별개인 곁가지 예산**이라
 * config 로 올리지 않고 여기 둔다 — 실패해도 fail-open(추천 채널만으로 진행)이고, 조정 근거가
 * 곧 이 주석이라 값과 근거를 떼어 놓을 이유가 없다(agent-query.ts 의 FALLBACK_TIMEOUT_MS 와 같은 결).
 *
 * 도구 없는 1턴이라 원래는 수십 초짜리지만, 카테고리 수만큼 채널을 나열하는 출력이 길고
 * (실측 5천 토큰대) 추론 모델은 그 앞에 생각 시간을 더 쓴다 — 120s 로는 실제로 초과했다.
 * 여기서 실패하면 발굴 채널이 통째로 빠져 RSS 후보가 급감하므로(실측 54건→9건) 여유를 준다.
 *
 * 실측(완료 로그 n=23): 중앙 40초 · p90 61초 · 최대 133초 — 240초는 최대치의 1.8배다.
 * 2026-08-02 00:15Z 에 한 번 초과했지만 그날은 절전 사고일이고 그 실행의 벽시계는 483초였다.
 * 예산이 유효 시간(deadline.ts) 기준으로 바뀐 지금 240초를 더 늘릴 근거는 실측에 없다.
 */
export const CHANNEL_DISCOVERY_TIMEOUT_MS = 240_000;
...
{"channels":{${defs.map((c) => `"${c.key}":["채널명 또는 @핸들"]`).join(",")}}}`;
  try {
    const text = await runAgentQueryText(
      ...
      CHANNEL_DISCOVERY_TIMEOUT_MS, // 근거는 상수 선언부 주석
      "유튜브 채널 발굴"
    );
...
export async function curateYoutube(date: string): Promise<YoutubeSnapshot> {
  ...
  try {
    ...
    const finalText = await runAgentQueryText(
      ...
      // 유튜브 전용 예산(기본 20분). 예전엔 공용 `agentQueryTimeoutMs`(60분)를 썼다.
      //
      // 실측(완료 로그 n=23): 중앙 192초 · p90 218초 · 최대 1993초. 최대값은 '오래 걸린 정상
      // 실행'이 아니라 maxTurns(120) 를 전부 소진하고 결과 47자만 남긴 폭주 런이다
      // (2026-07-26 05:18Z, 도구 86회). 그런 런은 더 기다려도 건질 게 없으므로 20분에서 끊고
      // catch-up 에 넘기는 편이 낫다. 앞단의 채널 발굴·RSS 하베스트까지 합쳐도 20분이면 넉넉하다.
      config.youtubeTimeoutMs,
      "유튜브 큐레이션"
    );
