...
function renderProduct(s: ProductSummary): string {
  const top3 = s.topListings
    .map((l) => `<li>${l.rank}. ${escapeHtml(l.mall)} — ${won(l.price)}</li>`)
    .join("");
  ...
  <div style="border:1px solid #eee;border-radius:10px;padding:16px;margin:12px 0">
    ...
    <table style="border-collapse:collapse;font-size:14px">
      <tr><td style="padding:2px 12px 2px 0;color:#666">종합 최저가</td><td><b>${won(s.latest?.overallLowest ?? null)}</b>${
    ...
    ${top3 ? `<div style="margin-top:8px"><b style="font-size:13px">Top3</b><ul style="margin:4px 0">${top3}</ul></div>` : ""}
...
export async function sendEmailReport(
  ...
  const body = summaries.map(renderProduct).join("");
  const html = `<div style="font-family:-apple-system,BlinkMacSystemFont,'Apple SD Gothic Neo',sans-serif;max-width:680px;margin:0 auto">
    ...
    <p style="color:#666;font-size:13px">대시보드: http://localhost:${config.port}</p>
    ${body}
