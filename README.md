...
7. **로또** — 예측 알고리즘이 어디까지 개선될 수 있는지 검증하는 **실험 탭**. 1회차(2002-12-07)부터 최신 회차까지 walk-forward 백테스트를 돌리며 전략을 세대별로 진화시킨다.

새 게시판은 `web/src/App.tsx`의 `TABS` 배열에 항목을 추가해 확장한다.

## 로또 예측 실험

수집 탭이 아니라 **실험 탭**이다. 매 회차 10세트(각 번호 6개)를 제출하고 실제 당첨번호와 대조하며, 50회차마다 전용 에이전트가 전략을 개정한다.

| 항목 | 내용 |
|---|---|
| 데이터 | 동행복권 `lt645/selectPstLt645InfoNew.do` (1회차~최신). **구 `getLottoNumber` API 는 WAF 로 차단되어 못 쓴다.** `srchDir=center` 는 요청 회차를 **중심으로** 위아래 10건을 준다(실측) |
| 대조 | 당첨번호 = 본번호 6 + **보너스 1 = 7개** |
| **목표** | **3+ 적중률 — 10세트 중 최소 1세트가 3개 이상 맞힌 회차의 비율** (아래) |
| 대조군 | 회차 점수(적중 m개=+m, 0개=-6, 10세트 합). 전략이 못 움직인다는 것이 확인돼 대조군으로 강등 |
...
| 토큰 한도 | 한도 소진 시 멈추고 **4시간 뒤 자동 재개**(`data/lotto-state.json`, 스케줄러가 30분마다 확인) |
| 시작 | 자동 아님 — 로또 탭의 **[실험 시작]** 버튼 |
| 반복 | **[실험 반복]** — 초기화 없이 한 바퀴 더. `run` 이 +1 되고 전략 세대와 이전 기록은 보존된다 |
| 완주 | `done` — **실험과 주간 동기화를 포함한 모든 자동 동작이 정지**하고 탭에 배너를 띄운다 |
...
⚠️ 천장과 최적 구조는 **에이전트 프롬프트에 넣지 않는다.** 답을 알려주면 실험이 아니라 받아쓰기가 된다. 화면에만 그려 사용자가 진척을 볼 수 있게 한다(테스트로 못박음).
...
| POST | `/api/lotto/repeat` | 실험 반복 (초기화 없이 한 바퀴 더) |
